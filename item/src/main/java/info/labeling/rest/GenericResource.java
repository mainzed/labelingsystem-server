...
public class GenericResource {
	...
	@GET
	@Path("/vocab/{vocabulary}")
	public Response getVocab(@HeaderParam("Accept") String acceptHeader, @PathParam("vocabulary") String itemID) throws IOException, JDOMException, ParserConfigurationException, TransformerException {
		try {
			String HOST_API = ConfigProperties.getPropertyParam("api");
			String HOST_HTMLPAGE = ConfigProperties.getPropertyParam("workbench") + "vocabularies/" + itemID + "/concepts";
			if (acceptHeader.startsWith("application/json")) {
	...
	@GET
	@Path("/concept/{label}")
	public Response getConcept(@HeaderParam("Accept") String acceptHeader, @PathParam("label") String itemID) throws IOException, JDOMException, ParserConfigurationException, TransformerException {
		try {
			// get vocabID
			String sparqlendpoint = ConfigProperties.getPropertyParam("api") + "sparql";
			String sparql = "PREFIX skos: <http://www.w3.org/2004/02/skos/core#> PREFIX dc: <http://purl.org/dc/elements/1.1/>"
					+ "SELECT ?vocid { "
					+ "<http://" + ConfigProperties.getPropertyParam("host") + "/item/concept/" + itemID + "> skos:inScheme ?scheme. "
					+ "?scheme dc:identifier ?vocid. "
					+ " }";
			URL obj = new URL(sparqlendpoint);
			HttpURLConnection con = (HttpURLConnection) obj.openConnection();
