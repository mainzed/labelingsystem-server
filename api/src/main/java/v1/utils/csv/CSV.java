...
public class CSV implements Runnable {
	...
	private static String createLabelTriples(String[] tokens, String creator, String context) throws IOException, UniqueIdentifierException, CsvLabelImportException, exceptions.ConfigException, exceptions.SesameSparqlException {
 ...
 /*if (sameAs.length >= 1 && !sameAs[0].equals("")) {
	...
	private static boolean vocabularyExistenceCheck(String voc, String creator) throws IOException, JDOMException, RepositoryException, MalformedQueryException, QueryEvaluationException, SesameSparqlException, ConfigException, CsvExistanceException, exceptions.ConfigException, exceptions.SesameSparqlException, SparqlQueryException, SparqlParseException {
		try {
			...
			List<BindingSet> voc_result = RDF4J_20.SPARQLquery(ConfigProperties.getPropertyParam("repository"),
	...
	public static String Input(String csvContent, String creator, String context, String filename, String filelink,
		...
		try {
			...
			if (!error) {
				...
				pw.close();
				if (!validator) {
					RDF4J_20.SPARQLupdate(ConfigProperties.getPropertyParam("repository"),
